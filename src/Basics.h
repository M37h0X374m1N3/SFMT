...
class MultiArray {
  public:
    static MultiArray init_ceil64(uint32_t n) { return MultiArray((n + 127) & ~63); }
  ...
  private:
    MultiArray(uint32_t ceil) : starts_(ceil), ends_(ceil), tags_(ceil), flags_(ceil), cap_(ceil) {}
...
class NodeArray {
  public:
    static NodeArray init_ceil64(uint32_t n) { return NodeArray((n + 64) & ~63); }
  ...
  private:
    NodeArray(uint32_t ceil)
        : tags_(ceil), main_(ceil), lhs_(ceil), rhs_(ceil), extra_(ceil), cap_(ceil),
          extra_cap_(ceil) {
