            ...
            switch (state) {
                case State::start: {
                    switch (c) {
                        ...
                        case '-': return {start, ++pos_, Tag::minus, flags};
                        case '+': return {start, ++pos_, Tag::plus, flags};
                        case '/': return {start, ++pos_, Tag::slash, flags};
                        case '*': return {start, ++pos_, Tag::asterisk, flags};
                        case '%': return {start, ++pos_, Tag::percent, flags};
                        case '=': return {start, ++pos_, Tag::equal, flags};
                        case '`': return {start, ++pos_, Tag::backtick, flags};
                        case '?': return {start, ++pos_, Tag::question, flags};
                        case '#': return {start, ++pos_, Tag::hash, flags};
                        case '!': return {start, ++pos_, Tag::bang, flags};
                        case '|': return {start, ++pos_, Tag::pipe, flags};
                        case '.': return {start, ++pos_, Tag::dot, flags};
                        case ':': return {start, ++pos_, Tag::colon, flags};
                        case ',': return {start, ++pos_, Tag::comma, flags};
                        case '~': return {start, ++pos_, Tag::tilde, flags};
                        case '&': return {start, ++pos_, Tag::ampersand, flags};
                        case '[': return {start, ++pos_, Tag::l_bracket, flags};
                        case ']': return {start, ++pos_, Tag::r_bracket, flags};
                        case '(': return {start, ++pos_, Tag::l_paren, flags};
                        case ')': return {start, ++pos_, Tag::r_paren, flags};
                        case '{': return {start, ++pos_, Tag::l_brace, flags};
                        case '}': return {start, ++pos_, Tag::r_brace, flags};
                ...
                case State::seen_dollar_sign: {
                    ...
                    return is_dollar(c) ? Token {start, ++pos_, Tag::double_dollar_sign, flags}
                ...
                case State::seen_dec_post: {
                    if (!is_ident_alnum(c)) { return {start, pos_, Tag::number, flags}; }
                    ++pos_;
                    state = is_hex(c) ? State::in_unprefixed_hex : State::in_num_malformed;
                    continue;
                }
                ...
                case State::seen_semicolon: {
                    ...
                    if (c == '\0') { return {pos_, pos_, Tag::eof, flags}; }
                    if (c == '\r') {
                        start = pos_;
                        state = State::seen_cr;
                    }
                ...
                case State::seen_angle_left: {
                    return Token {start, ++pos_,
                                  (c == '=' ? Tag::l_angle_equal : Tag::l_angle_bracket), flags};
                }

                case State::seen_angle_right: {
                    return Token {start, ++pos_,
                                  (c == '=' ? Tag::r_angle_equal : Tag::r_angle_bracket), flags};
                }
