...
/* NOTE: trying length bucket approach now but will want to switch to perfect
 * hash functions */
...
constexpr KwWords kw_pack(std::string_view s) { // pack bytes into W
    ...
    for (size_t i {}; i < s.size(); ++i) {
        k.w[i / 8] |= uint64_t(uint8_t(s[i])) << (8 * (i % 8));
    }
    return k;
}
...
alignas(16) constexpr KwTables kw = []() consteval {
    ...
    for (const auto &k : a)
        t.len_off[k.len + 1]++;
    for (size_t L {1}; L < t.len_off.size(); ++L)
        t.len_off[L] += t.len_off[L - 1];
    return t;
}();
